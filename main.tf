...
resource "aws_kms_key" "new_key" {
  ...
  description             = var.aws_kms_context.aws_kms_key_config.description
  enable_key_rotation     = var.aws_kms_context.aws_kms_key_config.rotation
...
resource "aws_iam_user" "account" {
  name          = "${var.aws_iam_context.aws_iam_user}-${random_pet.env.id}"
  path          = var.aws_iam_context.aws_iam_user_path
  force_destroy = true
  tags          = var.aws_iam_context.aws_iam_user_tags
}
...
resource "aws_iam_user_policy" "user-account-policy" {
  name   = "${var.aws_iam_context.aws_iam_user_policy_name}-${random_pet.env.id}"
  user   = aws_iam_user.account.name
  policy = jsonencode(var.aws_iam_context.aws_iam_user_policy) #data.aws_iam_policy_document.account-policy.json
}
